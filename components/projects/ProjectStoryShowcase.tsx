...
const ActionLink = memo(function ActionLink({ href, label, icon, theme }: ActionLinkProps) {
    const className = `${theme.action} ${theme.focusRing}`;

    if (!href) {
        return (
            <span aria-disabled="true" className={`${className} ${theme.actionDisabled}`}>
                {icon}
                {label}
            </span>
        );
...
function ProjectStoryShowcase({
    ...
    useLayoutEffect(() => {
        ...
        const context = gsap.context(() => {
            ...
            features.forEach((_, index) => {
                ...
                if (index > 0) {
                    timeline
                        .to(contents[index - 1], { autoAlpha: 0, y: -26, duration: 0.24 }, start)
                        .to(
                            images[index - 1],
                            { autoAlpha: 0, y: -18, scale: 0.97, duration: 0.28 },
                            start,
                        )
                        .to(numbers[index - 1], { color: theme.inactiveNumber, duration: 0.18 }, start)
                        .fromTo(
                            image,
                ...
                timeline
                    .to(number, { color: theme.accentColor, duration: 0.16 }, start + 0.02)
                    .fromTo(
                        content,
                ...
                timeline.to(image, { y: -8, scale: 1.006, duration: 0.62 }, start + 0.24);
            });
        }, story);
    ...
    const visual = (
        ...
        >
            <div className={`absolute -inset-4 hidden rounded-[2rem] border lg:block ${theme.imageOuterBorder}`} />
            <div className={`relative mx-auto aspect-[16/11] w-full max-w-full sm:max-w-[min(100%,560px)] overflow-hidden rounded-[1.35rem] border shadow-[0_36px_100px_rgba(0,0,0,0.18)] sm:rounded-[1.75rem] lg:max-w-none ${theme.imageFrame} ${theme.imageBackground}`}>
    ...
    const content = (
        <div className="flex flex-col justify-between lg:min-h-[calc(100vh-8rem)]">
            <div className="mb-7 flex items-center gap-3" aria-hidden="true">
                {features.map((feature, index) => (
                    <div key={feature.number} className="flex min-w-0 flex-1 items-center gap-3">
                        <span
                            ref={(node) => {
                        ...
                        <span className={`h-px min-w-0 flex-1 overflow-hidden rounded-full ${theme.progressTrack}`}>
                            <span
                                ref={(node) => {
            ...
            <div className="relative min-h-[280px] sm:min-h-[350px] lg:min-h-[430px]">
                {features.map((feature, featureIndex) => (
                    ...
                    >
                        <p data-stage-item className={`mb-5 text-xs font-medium uppercase tracking-[0.28em] ${theme.accentText}`}>
                            {feature.number} / {feature.eyebrow}
                        </p>
                        <h3 data-stage-item className={`mb-4 max-w-lg text-3xl font-medium tracking-tight text-balance sm:text-4xl lg:text-5xl ${theme.titleText}`}>
                            {feature.title}
                        </h3>
                        <p data-stage-item className={`mb-8 max-w-lg text-base leading-7 sm:text-lg ${theme.bodyText}`}>
                            {feature.description}
                        </p>
                        <ul className={`space-y-4 text-sm sm:text-base ${theme.bodyText}`}>
                            {feature.bullets.map((bullet) => (
                                <li key={bullet} data-stage-item className="flex items-start gap-3">
                                    <span className={`mt-1 text-sm leading-none ${theme.accentText}`}>✓</span>
                                    <span>{bullet}</span>
                                </li>
                            ))}
                        </ul>
            ...
            <div className={`mt-8 border-t pt-5 lg:sticky lg:bottom-8 ${theme.border}`}>
    ...
    return (
        <section className={`relative isolate w-full overflow-x-clip py-18 sm:py-24 ${theme.section}`} aria-labelledby={titleId}>
            {theme.background}
            {theme.transitionOverlay}

            <header className="mx-auto mb-14 w-[90%] max-w-6xl lg:mb-0 lg:pt-20">
                <div className={`max-w-3xl border-b pb-8 ${theme.headerBorder} ${layout === "right" ? "lg:ml-auto" : ""}`}>
                    <p className={`mb-5 text-sm font-medium ${theme.accentText}`}>{header.projectNumber}</p>
                    <h2 id={titleId} className={`mb-3 text-4xl font-medium tracking-tight text-balance sm:text-5xl ${theme.titleText}`}>
                        {header.projectName}
                    </h2>
                    <p className={`mb-5 text-sm uppercase tracking-[0.3em] ${theme.mutedText}`}>
                        {header.projectType}
                    </p>
                    <p className={`max-w-2xl text-base leading-7 sm:text-lg ${theme.bodyText}`}>
                        {header.projectDescription}
                    </p>
                    <p className={`mt-6 text-xs font-medium uppercase tracking-[0.24em] ${theme.mutedText}`}>
                        {header.projectTechStack}
                    </p>
                </div>
            </header>

            <div ref={storyRef} className="relative mx-auto min-h-[285vh] w-[90%] max-w-6xl lg:min-h-[320vh] motion-reduce:min-h-0">
...
export { ProjectStoryShowcase, type FeatureData, type HeaderData, type ProjectLinks, type ShowcaseTheme };
