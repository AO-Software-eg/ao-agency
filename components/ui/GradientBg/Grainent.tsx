...
const hexToRgb = (hex: string): [number, number, number] => {
  const h = hex.trim();
  if (h.startsWith("var(")) {
    const varName = h.slice(4, -1).trim();
    const resolved = (
      typeof document !== "undefined"
        ? getComputedStyle(document.documentElement).getPropertyValue(varName)
        : ""
    ).trim();
    if (!resolved) return [1, 1, 1];
    return hexToRgb(resolved);
  }
  if (h.startsWith("rgb")) {
    const inside = h.match(/rgba?\(([^)]+)\)/);
    if (inside) {
...
const Grainient: React.FC<GrainientProps> = ({
  ...
  useEffect(() => {
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        iTime: { value: 0 },
        iResolution: { value: new Float32Array([0.1, 0.1]) },
  ...
  useEffect(() => {
    ...
    if (
      typeof document !== "undefined" &&
      (color1.startsWith("var(") ||
        color2.startsWith("var(") ||
        color3.startsWith("var("))
    ) {
